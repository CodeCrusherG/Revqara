 ...
 *
 * A 1:1 port of `backend/ai_graph/packs.py`. The universal AI graph stays the
 * same for every tenant; the pack swaps lead fields, pipeline stages, intents,
 * qualification questions, response templates, tools, compliance rules, and
 * analytics.
 *
 * IMPORTANT: the insertion order of `keyword_stage_hints` keys is load-bearing
 * (first matching stage wins in `stageFor`). Do not reorder these objects.
 */
import type { Pack } from "../types";

/** Universal intents the classifier always understands. */
export const UNIVERSAL_INTENTS: readonly string[] = [
...
export const UNIVERSAL_PIPELINE: readonly string[] = [
  "new", "qualified", "contacted", "interested", "follow_up", "negotiation", "converted", "lost",
];

/**
 * Build a pack, layering overrides on top of the universal defaults — the TS
 * analogue of the Python `_pack(**kw)` factory.
 */
...
export const VERTICAL_PACKS: Record<string, Pack> = {
  custom: pack(),
  ...
  ecommerce: pack({
    ...
    templates: {
      ...
      follow_up: "Hi! Still thinking about your order? Here's a quick reminder — want me to share the link?",
  ...
  political_party: pack({
    vertical: "political_party",
